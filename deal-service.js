...
dealService.updateProductLink = async function updateProductLink(options) {
    try {
        ...
        const {product, site, link, newDeals, dryRun, runEnv} = options;
        ...
        const productId = _.get(productResult, 'content[0].productId');
        const productLinkCirrusLookup = `${cirrusApiPath}product_links?count=100000&format=json&joins=partnerUrl&productId=${productId}`;
        ...
        let productLinkId = _.get(productLink, '[0].niId');
        ...
        const body = Object.assign(adminLinkUriResult, {
            fromDate,
            update_type: "publish_campaign",
            deal: newFormattedDeals
        });
